...
public class AdminSchemaGenerator {
    ...
    public void buildSchema(PhysicalDataModel pdm) {
        ...
        pdm.addObject(fhirTablespace);

        addTenantSequence(pdm);
        addTenantTable(pdm);
        addTenantKeysTable(pdm);
        addVariable(pdm);
        ...
        IDatabaseObject allTablesComplete = new NopObject(adminSchemaName, "allTablesComplete");
        allTablesComplete.addDependencies(procedureDependencies);
        pdm.addObject(allTablesComplete);

        String templateFile = FhirSchemaConstants.SET_TENANT.toLowerCase() + ".sql";
        pdm.addProcedure(this.adminSchemaName, FhirSchemaConstants.SET_TENANT, FhirSchemaConstants.INITIAL_VERSION,
                () -> SchemaGeneratorUtil.readTemplate(adminSchemaName, adminSchemaName, templateFile, null),
     ...
     * @param pdm
     */
    public void addVariable(PhysicalDataModel pdm) {
     ...
     * @param model
     */
    protected void addTenantTable(PhysicalDataModel pdm) {
        Table tenantsTable =
        ...
        pdm.addTable(tenantsTable);
        pdm.addObject(tenantsTable);
    }
     ...
     * @param pdm
     */
    protected void addTenantKeysTable(PhysicalDataModel pdm) {
        Table tenantKeysTable =
        ...
        pdm.addTable(tenantKeysTable);
        pdm.addObject(tenantKeysTable);
    }
    ...
    protected void addTenantSequence(PhysicalDataModel pdm) {
        ...
        Sequence tenantSequence =
                new Sequence(adminSchemaName, TENANT_SEQUENCE, FhirSchemaConstants.INITIAL_VERSION, 1000);
