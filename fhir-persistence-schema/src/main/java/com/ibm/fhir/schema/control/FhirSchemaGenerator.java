...
public class FhirSchemaGenerator {
    ...
    public void buildAdminSchema(PhysicalDataModel model) {
        ...
        fhirTablespace =
                new Tablespace(FhirSchemaConstants.FHIR_TS, FhirSchemaConstants.INITIAL_VERSION,
        ...
        ProcedureDef setTenant =
                model.addProcedure(this.adminSchemaName,
                        SET_TENANT,
    ...
    public void addVariable(PhysicalDataModel model) {
        this.sessionVariable =
                new SessionVariableDef(adminSchemaName, "SV_TENANT_ID", FhirSchemaConstants.INITIAL_VERSION);
    ...
    public void buildSchema(PhysicalDataModel model) {
        ...
        ProcedureDef pd;
        pd =
                model.addProcedure(this.schemaName,
                        ADD_CODE_SYSTEM,
                        FhirSchemaConstants.INITIAL_VERSION,
                        () -> SchemaGeneratorUtil.readTemplate(adminSchemaName, schemaName,
                                ADD_CODE_SYSTEM.toLowerCase() + ".sql", null),
        ...
        pd =
                model.addProcedure(this.schemaName,
                        ...
                        FhirSchemaConstants.INITIAL_VERSION,
                        () -> SchemaGeneratorUtil.readTemplate(adminSchemaName, schemaName,
                                ADD_PARAMETER_NAME.toLowerCase() + ".sql", null),
        ...
        pd =
                model.addProcedure(this.schemaName,
                        ...
                        FhirSchemaConstants.INITIAL_VERSION,
                        () -> SchemaGeneratorUtil.readTemplate(adminSchemaName, schemaName,
                                ADD_RESOURCE_TYPE.toLowerCase() + ".sql", null),
        ...
        pd =
                model.addProcedure(this.schemaName,
                        ...
                        FhirSchemaConstants.INITIAL_VERSION,
                        () -> SchemaGeneratorUtil.readTemplate(adminSchemaName, schemaName,
                                ADD_ANY_RESOURCE.toLowerCase() + ".sql", null),
                        Arrays.asList(fhirSequence, resourceTypesTable, allTablesComplete),
                        procedurePrivileges);
        pd.addTag(SCHEMA_GROUP_TAG, FHIRDATA_GROUP);
    }
    ...
    public Table addResourceTokenValues(PhysicalDataModel pdm) {
        ...
        Table tbl =
                Table.builder(schemaName, tableName)
                        ...
                        .addForeignKeyConstraint(FK + tableName + "_LR", schemaName, LOGICAL_RESOURCES,
                                LOGICAL_RESOURCE_ID)
    ...
    public Table addResourceStrValues(PhysicalDataModel pdm) {
        ...
        Table tbl =
                Table.builder(schemaName, STR_VALUES)
                        .setTenantColumnName(MT_ID)
                        .addIntColumn(PARAMETER_NAME_ID, false)
                        ...
                        .addForeignKeyConstraint(FK + STR_VALUES + "_PNID", schemaName, PARAMETER_NAMES,
                                PARAMETER_NAME_ID)
                        .addForeignKeyConstraint(FK + STR_VALUES + "_RID", schemaName, LOGICAL_RESOURCES,
                                LOGICAL_RESOURCE_ID)
                        .setTablespace(fhirTablespace)
                        .addPrivileges(resourceTablePrivileges)
                        .enableAccessControl(this.sessionVariable)
    ...
    protected void addResourceTables(PhysicalDataModel model, IDatabaseObject... dependency) {
        checkValidSessionVariable(sessionVariable);

        // The sessionVariable is used to enable access control on every table, so we
        // provide it as a dependency
        FhirResourceTableGroup frg =
                new FhirResourceTableGroup(model, this.schemaName, sessionVariable, this.procedureDependencies,
                        this.fhirTablespace, this.resourceTablePrivileges);
        for (String resourceType : this.resourceTypes) {
            ...
            group.addDependencies(
                    Arrays.asList(this.codeSystemsTable, this.parameterNamesTable, this.resourceTypesTable));
    ...
    public void checkValidSessionVariable(SessionVariableDef var) {
        if (var == null) {
    ...
    protected void addParameterNames(PhysicalDataModel model) {
        ...
        parameterNamesTable =
                Table.builder(schemaName, PARAMETER_NAMES)
                        .setTenantColumnName(MT_ID)
                        .addIntColumn(PARAMETER_NAME_ID, false)
                        .addVarcharColumn(PARAMETER_NAME, 255, false)
                        .addUniqueIndex(IDX + "PARAMETER_NAME_RTNM", Arrays.asList(prfIndexCols),
                                Arrays.asList(prfIncludeCols))
    ...
    protected void addCodeSystems(PhysicalDataModel model) {
        codeSystemsTable =
                Table.builder(schemaName, CODE_SYSTEMS)
                        .setTenantColumnName(MT_ID)
                        .addIntColumn(CODE_SYSTEM_ID, false)
    ...
    protected String readResourceTemplate(String resourceType) {
        ...
        return SchemaGeneratorUtil.readTemplate(this.adminSchemaName, this.schemaName, ADD_RESOURCE_TEMPLATE,
                replacers);
    }
