...
-- Parameters:
--   p_logical_resource_id: the known primary key for the logical resource
--   p_new_resource: 1 if this is a newly created resource
--   p_resource_type: the resource type name
--   p_resource_type_id: the resource type id
--   p_logical_id: the logical id given to the resource by the FHIR server
--   p_payload:    the BLOB (of JSON) which is the resource content if any
...
-- Citus Distribed Function:
--   For Citus, we split the ingestion logic into two stored procedures (functions)
--   so that we can distribute these functions using the same distribution (sharding)
--   key as the tables they interact with
--   add_any_resource     - distributed by logical_id, which matches the sharding
--                          of logical_resource_shards
--   add_logical_resource - distributed by logical_resource_id, which matches the
--                          sharding of all the other tables used by statements in
--                          the procedure.
-- ----------------------------------------------------------------------------
    ( IN p_logical_resource_id           BIGINT,
      IN p_new_resource                     INT,
  ...
  IF p_new_resource = 1
  THEN
    -- create the corresponding entry in the 
    ...
    INSERT INTO {{SCHEMA_NAME}}.logical_resources (logical_resource_id, resource_type_id, logical_id, reindex_tstamp, is_deleted, last_updated, parameter_hash)
         VALUES (v_logical_resource_id, p_resource_type_id, p_logical_id, '1970-01-01', p_is_deleted, p_last_updated, p_parameter_hash_b64);
  ...
  END IF; -- end if new resource

  -- create the new resource version record
  EXECUTE
  ...
  INSERT INTO {{SCHEMA_NAME}}.resource_change_log(resource_id, change_tstamp, resource_type_id, logical_resource_id, version_id, change_type)
       VALUES (v_resource_id, p_last_updated, p_resource_type_id, v_logical_resource_id, p_version, v_change_type);
END $$;