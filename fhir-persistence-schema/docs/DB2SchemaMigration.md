...
#### Schema: Check objects in a Schema
**Query**
``` sql
select substr(OWNER,1,10) as OWNER,  substr(OBJECTSCHEMA,1,14) as OBJECTSCHEMA, substr(OBJECTNAME,1,30) as OBJECTNAME, OBJECTTYPE 
from SYSIBMADM.OBJECTOWNERS 
where OBJECTSCHEMA='FHIR_ADMIN' 
with ur
```
**Result**
```
OWNER      OBJECTSCHEMA   OBJECTNAME                     OBJECTTYPE
---------- -------------- ------------------------------ ------------------------
DB2INST1   FHIR_ADMIN     FHIR_ADMIN                     SCHEMA
DB2INST1   FHIR_ADMIN     SQL200227210218285             PROCEDURE
DB2INST1   FHIR_ADMIN     TENANT_KEYS                    TABLE
  3 record(s) selected.
```

#### Schema: Check Tablespace needs altering
**Query**
``` sql
SELECT varchar(tbsp_name, 30) as tbsp_name, 
  tbsp_free_pages - (tbsp_usable_pages - tbsp_page_top) AS freespaceBelowHWM
FROM TABLE(MON_GET_TABLESPACE('',-2)) AS t 
ORDER BY freespaceBelowHWM DESC
```
**Result**
```
TBSP_NAME  freespaceBelowHWM
---------- -------------- 
TSTENANT1	670080
FHIR_TS	30432
TS_TENANT5	1392
...
#### Schema: Check the Object Types in a Schema
**Query**
``` sql
select DISTINCT OBJECTTYPE
from SYSIBMADM.OBJECTOWNERS
where OBJECTSCHEMA in ('FHIR_ADMIN')
with ur;
```
**Result** 
``` sql
...
#### Schema: Check if an object is managed in VERISON_HISTORY
**Query**
``` sql
SELECT * FROM FHIR_ADMIN.VERSION_HISTORY WHERE SCHEMA_NAME = 'FHIR_ADMIN' AND OBJECT_NAME = 'SV_TENANT_ID'
```
**Result**
``` sql
SCHEMA_NAME OBJECT_TYPE OBJECT_NAME  VERSION APPLIED
----------- ----------- ------------ ------- ----------------------------
FHIR_ADMIN  VARIABLE    SV_TENANT_ID 1       2020-02-27-21.02.17.561092
  1 record(s) selected.
```
