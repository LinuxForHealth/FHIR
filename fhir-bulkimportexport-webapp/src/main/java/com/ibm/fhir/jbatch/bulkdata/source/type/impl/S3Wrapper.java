 ...
 *
 */
public class S3Wrapper implements SourceWrapper {
    ...
    public S3Wrapper(String source) throws FHIRException {
        ConfigurationAdapter adapter = ConfigurationFactory.getInstance();

        boolean useFhirServerTrustStore = adapter.shouldCoreCosUseServerTruststore();

        String cosLocation = adapter.getSourceLocation(source);
        String cosEndpointUrl = adapter.getSourceEndpointInternal(source);
        String bucketName = adapter.getSourceBucketName(source);

        boolean iam = adapter.isSourceAuthTypeIam(source);
        isExportPublic = adapter.isSourceExportPublic(source);

        String apiKey = null;
        String resourceId = null;
        String authType = adapter.getSourceAuthType(source);
        if (authType == null) {
            logger.warning("S3Wrapper: No Auth Type Found");
        } else if ("hmac".equalsIgnoreCase(authType)) {
            apiKey = adapter.getSourceAuthTypeHmacAccessKey(source);
            resourceId = adapter.getSourceAuthTypeHmacSecretKey(source);
        } else if ("basic".equalsIgnoreCase(authType)) {
            apiKey = adapter.getSourceAuthTypeUsername(source);
            resourceId = adapter.getSourceAuthTypePassword(source);
        } else if ("iam".equalsIgnoreCase(authType)) {
            apiKey = adapter.getSourceAuthTypeIamApiKey(source);
            resourceId = adapter.getSourceAuthTypeIamApiResourceInstanceId(source);
        }
        ...
        create = adapter.shouldSourceCreate(source);
        ...
        if (adapter.isSourceParquetEnabled(source)) {
            try {
                Class.forName("org.apache.spark.sql.SparkSession");
                parquetWriter = new SparkParquetWriter(adapter.isSourceAuthTypeIam(source), cosEndpointUrl, apiKey, resourceId);
