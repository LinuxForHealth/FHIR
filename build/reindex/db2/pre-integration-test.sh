...
DIST="${WORKSPACE}/build/reindex/kafka/workarea/volumes/dist"
...
config(){
    ...
    mkdir -p ${DIST}/
    mkdir -p ${WORKSPACE}/build/reindex/kafka/workarea/output

    touch ${WORKSPACE}/build/reindex/kafka/workarea/output/fhir_reindex-messages.log
    chmod +rwx ${WORKSPACE}/build/reindex/kafka/workarea/output/fhir_reindex-messages.log
    chmod -R 777 ${WORKSPACE}/build/reindex/kafka/workarea/output/
    ...
    cp -pr ${WORKSPACE}/build/reindex/kafka/resources/* ${WORKSPACE}/build/reindex/kafka/workarea/volumes/dist/config/default/
    mv ${WORKSPACE}/build/reindex/kafka/workarea/volumes/dist/config/default/fhir-server-config-reindex-cicd.json ${WORKSPACE}/build/reindex/kafka/workarea/volumes/dist/config/default/fhir-server-config.json
}
...
bringup(){
    ...
    # Create the FHIR_reindex topic
    docker-compose -f build/reindex/kafka/docker-compose.yml exec kafka-1 bash /bin/kafka-topics \
        --bootstrap-server kafka-1:19092,kafka-2:29092 --command-config /etc/kafka/secrets/client-ssl.properties \
        --create --topic FHIR_reindex --partitions 10 --replication-factor 2
    [ $? -eq 0 ] || exit 9

    echo "Topic is created 'FHIR_reindex'"
    exit 0
}
