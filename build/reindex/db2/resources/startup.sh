...
if [ ! -f /opt/fhir-server/tools/tenant.key ]
then
    echo 'change-password' > /opt/fhir-server/tools/tenant.key
    ...
    java -jar /opt/fhir-server/tools/fhir-persistence-schema-*-cli.jar \
        --db-type db2 --prop db.host=db2 --prop db.port=50000 --prop db.database=fhirdb --prop user=db2inst1 --prop password=change-password \
        --allocate-tenant default --tenant-key-file /opt/fhir-server/tools/tenant.key --grant-to fhirserver
fi
