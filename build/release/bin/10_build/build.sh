...
function _mvn2 {
    ...
    # Batch mode without the transfer updates.
    mvn ${THREAD_COUNT}  "${PROFILES}" install -DskipTests -f ${PROJECT_PATH} -Dmaven.wagon.http.retryHandler.count=3
    check_and_fail $? "${FUNCNAME[0]} - stopped - ${PROJECT_PATH}"
}

# build_all - build all versions
function build_all {
    _mvn 'fhir-tools' '-Pdeploy-to-mc,fhir-javadocs'
    _mvn 'fhir-examples' '-Pdeploy-to-mc,fhir-javadocs'
    _mvn2 'fhir-parent' '-Pdeploy-to-mc'
    ...
    _mvn 'fhir-parent' "-Pdeploy-to-mc,fhir-javadocs,fhir-validation-distribution,fhir-ig-carin-bb,fhir-ig-davinci-pdex-formulary,fhir-ig-davinci-pdex-plan-net,fhir-ig-davinci-hrex,fhir-ig-davinci-pdex,fhir-ig-mcode,fhir-ig-us-core,fhir-term,${PROFILES}"
}
