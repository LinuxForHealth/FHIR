...
clean_up(){
    # Remove problematic resource types with known backwards-incompatibilities
    fhir_base_url=https://localhost:9443/fhir-server/api/v4
    problem_types=DeviceDefinition,Evidence,EvidenceVariable
      ...
      curl -i -k -u fhiruser:change-password -H "Content-Type: application/json" ${r}'/$erase' -d '
        {
          ...
          "parameter": [
            ...
            {
              "name": "reason",
              "valueString": "remove before migrating to R4B"
            }
          ]
        }'
    done
