...
package com.ibm.fhir.ig.davinci.pdex.formulary.util;
...
public class IndexGenerator {
    public static void main(String[] args) throws Exception {
        buildAndWriteIndex("hl7/fhir/us/davinci-pdex-formulary");
    }
    ...
    private static void buildAndWriteIndex(String packagePath) throws IOException, FileNotFoundException {
        ...
        for (File file : dir.listFiles()) {
            if (!file.isDirectory() && !file.getName().startsWith(".")
                    && !file.getName().startsWith("CHANGELOG.md")
                    && !file.getName().startsWith("spec.internals")) {
