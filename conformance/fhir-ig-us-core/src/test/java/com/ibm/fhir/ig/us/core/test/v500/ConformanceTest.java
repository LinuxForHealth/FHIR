...
public class ConformanceTest {
    ...
    public void testConformsToWithEmptyContext() throws Exception {
        try (InputStream in = ConformanceTest.class.getClassLoader().getResourceAsStream("JSON/500/tests/us-core-patient.json")) {
    ...
    public void testUSCoreMedicationRequest() throws Exception {
        ...
        try (Reader r = new InputStreamReader(getClass().getClassLoader().getResourceAsStream("JSON/500/MedicationRequest-uscore-mo2.json"))) {
    ...
    public void testUSCorePulseOximetry() throws Exception {
         ...
         */
        try (Reader r = new InputStreamReader(getClass().getClassLoader().getResourceAsStream("JSON/500/Observation-satO2-fiO2.json"))) {
            Resource resource = FHIRParser.parser(Format.JSON).parse(r);
            List<Issue> issues = FHIRValidator.validator().validate(resource);
            issues.forEach(item -> {
                if (item.getSeverity().getValue().equals("error")) {
    ...
    public void testUSCoreCarePlan() throws Exception {
        InputStream in = getClass().getClassLoader().getResourceAsStream("JSON/500/tests/us-core-careplan.json");
    ...
    public void testUSCoreValidation1() throws Exception {
        try (InputStream in = getClass().getClassLoader().getResourceAsStream("JSON/500/tests/us-core-patient-no-name-asserted.json")) {
            Patient patient = FHIRParser.parser(Format.JSON).parse(in);
            List<Issue> issues = FHIRValidator.validator().validate(patient);
            issues.forEach(System.out::println);
    ...
    public void testUSCoreValidation2() throws Exception {
        try (InputStream in = getClass().getClassLoader().getResourceAsStream("JSON/500/tests/us-core-patient-no-name-not-asserted.json")) {
            Patient patient = FHIRParser.parser(Format.JSON).parse(in);
            List<Issue> issues = FHIRValidator.validator().validate(patient);
            issues.forEach(System.out::println);
            Assert.assertEquals(countErrors(issues), 0);
    ...
    public void testUSCoreValidation5() throws Exception {
        try (InputStream in = getClass().getClassLoader().getResourceAsStream("JSON/500/tests/Pamela954_Johns824_4818eca9-c6d2-4fa0-a234-7244e620391e.json")) {
    ...
    public void testUSCoreValidation6() throws Exception {
        try (InputStream in = getClass().getClassLoader().getResourceAsStream("JSON/500/tests/1.json")) {
    ...
    public void testUSCoreValidateAddress() throws Exception {
        ...
        Assert.assertEquals(issues.get(0).getDetails().getText().getValue(), "Code 'bogus' is not a valid member of ValueSet "
                + "with URL=http://hl7.org/fhir/us/core/ValueSet/us-core-usps-state and version=5.0.0");
