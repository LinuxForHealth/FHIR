...
public class ProfileTest {
    ...
    public void testUSCoreValidation() throws Exception {
        try (Reader r = ExamplesUtil.resourceReader(path)) {
            System.out.println("US Core Testing -> " + path);
    ...
    public Object[] createInstances() {
        ...
        try (BufferedReader br = new BufferedReader(ExamplesUtil.indexReader(Index.PROFILES_US_CORE_JSON))) {
            String line;
        ...
        try (BufferedReader br = new BufferedReader(ExamplesUtil.indexReader(Index.PROFILES_US_CORE_XML))) {
            String line;
            while ((line = br.readLine()) != null) {
                String[] tokens = line.split("\\s+");
                if (tokens.length == 2) {
                    String expectation = tokens[0];
                    String example = tokens[1];
                    result.add(new ProfileTest(expectation, example, false));
                }
            }
        } catch (IOException e) {
            e.printStackTrace();
