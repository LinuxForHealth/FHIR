...
public class Base64BinaryTest extends FHIRServerTestBase {
    ...
    public void testCreateStructureDefn() throws Exception {
        ...
        List<ElementDefinition> listElementDef = new ArrayList<ElementDefinition>();
        listElementDef
                .add(ElementDefinition.builder(string("/Patient")).definition(Markdown.of("y")).min(UnsignedInt.of(1))
                        .max(string("1")).type(ElementDefinition.Type.builder(Uri.of("x")).build())
                        .base(Base.builder(string("Patient/1"), UnsignedInt.of(1), string("1")).build())
                        .defaultValue(Base64Binary.builder().value(valueSnapshot).build())
                        .example(Example.builder(string("ExampleBase64Binary"),
                                Base64Binary.builder().value(valueSnapshot).build()).build())
                        .fixed(Base64Binary.builder().value(valueSnapshot).build())
                        .maxValue(Base64Binary.builder().value(valueSnapshot).build())
                        .minValue(Base64Binary.builder().value(valueSnapshot).build())
                        .pattern(Base64Binary.builder().value(valueSnapshot).build()).build());

        StructureDefinition struct = StructureDefinition
                .builder(Uri.of("http://hl7.org/fhir/test"), string("createStructureDefnTest"), PublicationStatus.DRAFT,
                        StructureDefinitionKind.LOGICAL, com.ibm.watsonhealth.fhir.model.type.Boolean.TRUE,
                        Uri.builder().value("/Patient").build())
                .snapshot(Snapshot.builder(listElementDef).build()).build();
        ...
        assertEquals(
                new String(
                        ((Base64Binary) responseStruct.getSnapshot().getElement().get(0).getDefaultValue()).getValue()),
                valueSnapshotString);
        assertEquals(
                new String(((Base64Binary) responseStruct.getSnapshot().getElement().get(0).getExample()).getValue()),
                valueSnapshotString);
        assertEquals(
                new String(((Base64Binary) responseStruct.getSnapshot().getElement().get(0).getFixed()).getValue()),
                valueSnapshotString);
        assertEquals(
                new String(((Base64Binary) responseStruct.getSnapshot().getElement().get(0).getMaxValue()).getValue()),
                valueSnapshotString);
        assertEquals(
                new String(((Base64Binary) responseStruct.getSnapshot().getElement().get(0).getMinValue()).getValue()),
                valueSnapshotString);
        assertEquals(
                new String(((Base64Binary) responseStruct.getSnapshot().getElement().get(0).getPattern()).getValue()),
                valueSnapshotString);
