...
The `fhirServer/searchParameterFilter` property is a JSON map where the key represents the resource type, and the value is an array of strings representing search parameter codes. The wildcard (`"*"`) can be used either as a resource type or as a search parameter code.

Filtering can only be controlled at the Resource level on which it is defined. For example, for SearchParameters defined on the resource type `Resource` (i.e. applicable to all Resources), the filter must be applied with a key of `Resource`:
