...
package com.ibm.watson.health.fhir.swagger.generator;
...
public class FHIRSwaggersGenerator {
...
//        filter = createFilter("CapabilityStatement(read)");
        ...
        for (String resourceClassName : classNames) {
            ...
            if (DomainResource.class.isAssignableFrom(resourceModelClass) && filter.acceptResourceType(resourceModelClass)) {
                ...
                info.add("description", "The IBM Watson Health FHIR Server API for " + resourceClassName + " resources.");
                info.add("version", "4.0.0");
                swagger.add("info", info);

                swagger.add("basePath", "/wh-fhir-dev/api/v4");

                addApiConnectStuff(swagger);
                ...
                // generate DomainResource definitions
...
//        paths.add("/", path);
    ...
    private static void addApiConnectStuff(JsonObjectBuilder swagger) {
    ...
    private static JsonObjectBuilder buildProperties() {
        JsonObjectBuilder targetUrl = factory.createObjectBuilder()
                                            .add("value", "https://us-south.wh-fhir.dev.cloud.ibm.com/wh-fhir-dev/api/v4/")
    ...
    private static void populateStructureDefinitionMap(Map<Class<?>, StructureDefinition> structureDefinitionMap,
        ...
        InputStream stream = FHIRSwaggersGenerator.class.getClassLoader().getResourceAsStream(structureDefinitionFile);
