...
/**
 * This class implements a tenant-specific cache that holds PropertyGroup objects
 * (i.e. FHIR Server configuration information).
 * Each cache entry holds the in-memory representation of the fhir-server-config.json file
 * found for a particular tenant.
 * 
 * @author padams
 */
public class TenantSpecificPropertyGroupCache extends TenantSpecificFileBasedCache<PropertyGroup> {
    ...
    public String getCacheEntryFilename(String tenantId) {
        return FHIRConfiguration.getConfigHome() + FHIRConfiguration.CONFIG_LOCATION 
                + File.separator + tenantId + File.separator + FHIRConfiguration.CONFIG_FILE_BASENAME;
    }
