...
public class InclusionQuerySegmentAggregator extends QuerySegmentAggregator {
	 ...
	 * /Observation?id=some-id&_include=Observation:patient
	 * See comments embedded in the SQL for explanation:
	 * 
	   SELECT COUNT(RESOURCE_ID) FROM 
	   -- The following SELECT is the query for the "target" Observation resource. It is generated by superclass methods.
	  (SELECT R.RESOURCE_ID, R.LOGICAL_RESOURCE_ID, R.VERSION_ID, R.LAST_UPDATED, R.IS_DELETED, R.DATA, LR.LOGICAL_ID FROM 
	   ...
	   Observation_TOKEN_VALUES P1 ON P1.RESOURCE_ID=R.RESOURCE_ID WHERE 
	   R.IS_DELETED <> 'Y' AND 
	   ...
	   -- The following SELECT is generated in order to retrieve the desired included Patient resources that are
	   -- referenced by the target Observation resources.
	   SELECT R.RESOURCE_ID, R.LOGICAL_RESOURCE_ID, R.VERSION_ID, R.LAST_UPDATED, R.IS_DELETED, R.DATA, LR.LOGICAL_ID FROM 
	   Patient_RESOURCES R JOIN Patient_LOGICAL_RESOURCES LR ON R.LOGICAL_RESOURCE_ID=LR.LOGICAL_RESOURCE_ID WHERE 
	   R.IS_DELETED <> 'Y' AND 
	   R.RESOURCE_ID = LR.CURRENT_RESOURCE_ID AND 
	   -- We're looking for Patients whose logical ids are in the Observation_STR_Values table, in the form
	   -- 'Patient/logicalId'.
	   ('Patient/' || LR.LOGICAL_ID IN 
	     (SELECT P1.STR_VALUE FROM Observation_STR_VALUES P1 WHERE 
	      P1.PARAMETER_NAME_ID=14 AND P1.RESOURCE_ID IN 
	        -- Here in this SELECT, we repeat the original query for the target Observations, so the set of 
	        -- Observations is narrowed to only those the meet the target search criteria.
	        (SELECT R.RESOURCE_ID FROM Observation_RESOURCES R JOIN 
	         Observation_LOGICAL_RESOURCES LR ON R.LOGICAL_RESOURCE_ID=LR.LOGICAL_RESOURCE_ID  JOIN 
	         Observation_TOKEN_VALUES P1 ON P1.RESOURCE_ID=R.RESOURCE_ID WHERE 
	         R.IS_DELETED <> 'Y' AND 
	         P1.RESOURCE_ID = R.RESOURCE_ID AND 
	...
	private void processIncludeParameters(StringBuilder queryString, List<Object> bindVariables) throws Exception {
		...
		for (InclusionParameter includeParm : this.includeParameters) {
			...
			queryString.append(QuerySegmentAggregator.SELECT_ROOT);
			// FROM Patient_RESOURCES R JOIN Patient_LOGICAL_RESOURCES LR ON R.LOGICAL_RESOURCE_ID=LR.LOGICAL_RESOURCE_ID
	...
	private void processRevIncludeParameters(StringBuilder queryString, List<Object> bindVariables) throws Exception { 
		...
		//TODO implementation to be done in a future sprint.
	}
