...
package com.ibm.watson.health.database.utils.api;
...
import com.ibm.watson.health.database.utils.model.PartitionDef;
...
/**
 * Abstraction of the SQL to use for a given database. This allows us to
 * hide as much as possible the differences in syntax and support between DB2 and
 * Derby (which is used for unit-testing). Derby is pretty close to DB2
 * in most cases, but notably does not support partitioning, variables
 * or SPL stored procedures.
 * @author rarnold
 *
 */
public interface IDatabaseAdapter {
     ...
     * @param partitionDef
     */
    public void createTable(String schemaName, String name, String tenantColumnName, List<ColumnBase> columns, PrimaryKeyDef primaryKey, String tablespaceName);
    ...
    /**
     * Create ARRAY type used for passing values to stored procedures e.g.:
    ...
    /**
     * Deactivate row access control on a table
       ALTER TABLE <tbl> DEACTIVATE ROW ACCESS CONTROL 
     * @param schemaName
     * @param tableName
     */
    ...
    /**
     * Detach the partitions from each of the given tables. The tenantStaingTable is the name
     * of the table used to record the "into" table name that the detached partition becomes.
     * @param tables
     * @param schemaName
     * @param newTenantId
     */
    public void removeTenantPartitions(Collection<Table> tables, String schemaName, int tenantId, String tenantStagingTable);
    ...
    /**
     * Grant the list of privileges on the named object to the user. This is a general purpose
     * method which can be used to specify privileges for any object type which doesn't need
     * the object type to be specified in the grant DDL.
     * @param schemaName
     * @param tableName
     * @param privileges
     * @param toUser
