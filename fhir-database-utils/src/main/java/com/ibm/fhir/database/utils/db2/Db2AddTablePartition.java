...
public class Db2AddTablePartition implements IDatabaseStatement {
    ...
    public void run(IDatabaseTranslator translator, Connection c) {
        if (DryRunContainer.getSingleInstance().isDryRun()) {
            DryRunContainer.getSingleInstance().add(buildSqlString(), null);
        } else {
    ...
    protected String buildSqlString() {
        ...
        return "ALTER TABLE " + schemaName + "." + tableName
                + " ADD PARTITION " + partitionName + " STARTING FROM " + lowValueStr + " INCLUSIVE  ENDING AT "
                + highValueStr + " INCLUSIVE"
