...
public abstract class BaseObject implements IDatabaseObject {
    ...
    @Override
    public ITaskGroup collect(final ITaskCollector tc, final IDatabaseAdapter target, final ITransactionProvider tp,
            final IVersionHistoryService vhs) {
        ...
        while (remainingAttempts-- > 0) {
            try (ITransaction tx = tp.getTransaction()) {
                try {
                    ...
                    // tried again.
                    if (x.isDeadlock()) {
                        logger.warning("Deadlock detected processing: " + this.getTypeAndName() + " [remaining="
                                + remainingAttempts + "]");
                    } else {
                        logger.warning("Lock timeout detected processing: " + this.getTypeAndName() + " [remaining="
                                + remainingAttempts + "]");
                    }
                    tx.setRollbackOnly();
