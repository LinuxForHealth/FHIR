...
import com.ibm.watson.health.fhir.persistence.test.common.AbstractPersistenceTest;
...
public abstract class AbstractPLSearchTest extends AbstractPersistenceTest{
    ...
    protected Composition createCompositionReferencingSavedResource() throws Exception {
    	...
    	Reference ref = Reference.builder()
    			.reference(com.ibm.watson.health.fhir.model.type.String.of("Basic/" + savedResource.getId().getValue()))
    			.build();
    	
    	composition = Composition.builder()
    	        .status(CompositionStatus.builder().value(CompositionStatus.ValueSet.PRELIMINARY).build())
    	        .category(CodeableConcept.builder().text(com.ibm.watson.health.fhir.model.type.String.of("test")).build())
    	        .date(DateTime.of("2019"))
    	        .author(Arrays.asList(ref)).title(com.ibm.watson.health.fhir.model.type.String.of("TEST")).build();
