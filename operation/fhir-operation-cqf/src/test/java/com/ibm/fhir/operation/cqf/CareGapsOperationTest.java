...
public class CareGapsOperationTest {
    ...
    public void testDoEvaluationEXM74() throws Exception {
        ...
        Parameters.Parameter pPeriodStart = Parameters.Parameter.builder().name(fhirstring(CareGapsOperation.PARAM_IN_PERIOD_START)).value(Date.of(periodStart)).build();
        Parameters.Parameter pPeriodEnd = Parameters.Parameter.builder().name(fhirstring(CareGapsOperation.PARAM_IN_PERIOD_END)).value(Date.of(periodEnd)).build();
        ...
        Parameters.Parameter pSubject = Parameters.Parameter.builder().name(fhirstring(CareGapsOperation.PARAM_IN_SUBJECT)).value(fhirstring("Patient/" + patient.getId())).build();
        ...
        try (MockedStatic<FHIRRegistry> staticRegistry = mockStatic(FHIRRegistry.class)) {
            ...
            Parameters result = operation.doInvoke(
            ...
            assertSame( 1, bundle.getTotal().getValue() );
        }
    }
