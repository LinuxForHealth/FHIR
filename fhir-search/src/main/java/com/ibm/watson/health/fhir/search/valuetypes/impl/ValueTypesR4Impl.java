...
package com.ibm.watson.health.fhir.search.valuetypes.impl;
...
import com.ibm.watson.health.fhir.search.valuetypes.IValueTypes;
import com.ibm.watson.health.fhir.search.valuetypes.cache.TenantSpecificValueTypesCache;
...
public class ValueTypesR4Impl implements IValueTypes {
    ...
    public boolean isDateSearch(Class<?> resourceType, Parameter queryParm) throws FHIRSearchException {
        ...
        return Type.TOKEN.compareTo(queryParm.getType()) == 0 || valueTypes.contains(com.ibm.watson.health.fhir.model.type.Date.class)
    ...
    public boolean isIntegerSearch(Class<?> resourceType, Parameter queryParm) throws FHIRSearchException {
        try {
            Set<Class<?>> valueTypes = getValueTypes(resourceType, queryParm.getName());
            return (valueTypes.contains(com.ibm.watson.health.fhir.model.type.Integer.class) || valueTypes.contains(UnsignedInt.class)
    ...
    public Set<Class<?>> convertToClasses(JsonArray classesArray) {
        ...
        while (iter.hasNext()) {
            ...
            boolean found = processClass(clzs, "com.ibm.watson.health.fhir.model.resource", type);
            if (!found) {
                // Checks the other possibility
                found = processClass(clzs, "com.ibm.watson.health.fhir.model.type", type);
            }
